let voiceCacheFix={en:[],pt:[]}; let vReady=false; function refreshV(){ const a=speechSynthesis.getVoices(); if(!a.length) return; vReady=true; voiceCacheFix.en=a.filter(v=>v.lang.toLowerCase().startsWith('en')); voiceCacheFix.pt=a.filter(v=>v.lang.toLowerCase().startsWith('pt')); } refreshV(); if(speechSynthesis.onvoiceschanged!==undefined) speechSynthesis.onvoiceschanged=refreshV; function getEN(){ if(!vReady) refreshV(); return voiceCacheFix.en.find(v=>v.lang==='en-US')||voiceCacheFix.en[0]||null; } function getPT(){ if(!vReady) refreshV(); return voiceCacheFix.pt.find(v=>v.lang==='pt-BR')||voiceCacheFix.pt[0]||null; } function speakENFix(t,r=0.9,o){ if(!t) return; speechSynthesis.cancel(); setTimeout(()=>{ const u=new SpeechSynthesisUtterance(t); u.lang='en-US'; u.rate=r; const v=getEN(); if(v) u.voice=v; if(o) u.onend=o; speechSynthesis.speak(u); },80); } function speakPTFix(t,r=1,o){ if(!t) return; const pure=/^[A-Za-z0-9 .,!?'-]+$/.test(t)&&!/[áàâãéêíóôõúç]/.test(t)&&/\b(hello|my name|I am|would like|coffee|please)\b/i.test(t); if(pure){ speakENFix(t,r,o); return; } speechSynthesis.cancel(); setTimeout(()=>{ const u=new SpeechSynthesisUtterance(t); u.lang='pt-BR'; u.rate=r; const v=getPT(); if(v) u.voice=v; if(o) u.onend=o; speechSynthesis.speak(u); },80); } window.speakEnglish=speakENFix; window.speakPortuguese=speakPTFix; window.speakBilingual=(en,pt)=>{ speakENFix(en,0.9,()=>{ setTimeout(()=> speakPTFix(pt,1.0),600); }); };\n/* Guided speaking exercise: checks a spoken answer, corrects it, and moves ahead. */
(function () {
  ...
  function say(text, afterSpeaking) {
    ...
    if (afterSpeaking) speech.onend = afterSpeaking;
